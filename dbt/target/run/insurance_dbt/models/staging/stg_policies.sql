  ...
  create view "insurance_db"."staging_staging"."stg_policies__dbt_tmp"
...
with source_data as (
    select * from "insurance_db"."raw_data"."policies"
),
...
cleaned_data as (
    select
        ...
        case 
            when premium_amount <= 1000 then 'Basic'
            when premium_amount <= 3000 then 'Standard'
            else 'Premium'
        end as premium_tier,
...
final as (
    select 
        *,
        ...
        case 
            when policy_status = 'active' and premium_amount > 3000 then 'High Value Active'

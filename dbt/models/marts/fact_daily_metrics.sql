{{
  config(
    ...
    unique_key=['metric_date', 'policy_type', 'state_code'],
...
with daily_policy_metrics as (
    ...
    inner join {{ ref('stg_customers') }} c on p.customer_id = c.customer_id
    where date_trunc('day', p.created_at)::date = '{{ var("execution_date") }}'
    
    {% if is_incremental() %}
        and date_trunc('day', p.created_at)::date >= (select max(metric_date) from {{ this }}) - interval '{{ var("lookback_days") }} days'
    {% endif %}
...
daily_claims_metrics as (
    ...
    inner join {{ ref('stg_policies') }} p on cl.policy_id = p.policy_id
    inner join {{ ref('stg_customers') }} c on p.customer_id = c.customer_id
    where date_trunc('day', cl.reported_date)::date = '{{ var("execution_date") }}'
    
    {% if is_incremental() %}
        and date_trunc('day', cl.reported_date)::date >= (select max(metric_date) from {{ this }}) - interval '{{ var("lookback_days") }} days'
    {% endif %}
...
daily_payment_metrics as (
    ...
    from {{ source('raw_data', 'payments') }} py
    inner join {{ ref('stg_policies') }} p on py.policy_id = p.policy_id
    inner join {{ ref('stg_customers') }} c on p.customer_id = c.customer_id
    where date_trunc('day', py.payment_date)::date = '{{ var("execution_date") }}'
    
    {% if is_incremental() %}
        and date_trunc('day', py.payment_date)::date >= (select max(metric_date) from {{ this }}) - interval '{{ var("lookback_days") }} days'
    {% endif %}
...
portfolio_metrics as (
    select 
        '{{ var("execution_date") }}'::date as metric_date,
        p.policy_type,
        c.state_code,
    ...
    from {{ ref('stg_policies') }} p
    inner join {{ ref('stg_customers') }} c on p.customer_id = c.customer_id
    where '{{ var("execution_date") }}'::date between p.effective_date and p.expiration_date
    group by 1, 2, 3
),
...
final as (
    select 
        ...
        '{{ var("execution_date") }}' as load_date
